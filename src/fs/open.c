...
PUBLIC int do_open()
{
	...
	if (flags & O_CREAT) {
		...
		if (inode_nr) {
			...
			printf("pathname before create:%s\n", pathname);
			pin = create_file(pathname, flags);//need add operation about directory
			/*if(pin)
...
PUBLIC int do_open_dir()
{
	...
	if (flags & O_CREAT) {
		...
		if (inode_nr) {
			...
			pin = create_dir(pathname, flags);//need add operation about directory
			char filename[MAX_PATH];
			struct inode * dir_inode;
			if (strip_path(filename, pathname, &dir_inode) != 0) {
				return -1;
...
PRIVATE struct inode * create_file(char * path, int flags)
{
	char filename[MAX_PATH];
	struct inode * dir_inode;
	//printf("%s\n", "22222222222222222222222227");
	//printf("{{%s}}\n", path);
	if (strip_path(filename, path, &dir_inode) != 0) {
		printf("{%s,%s,%d}\n", filename,path,dir_inode->i_num);
		printl("%s\n", "when create file second strip_path != 0");
		return 0;
	}
	//printf("%s\n", "==0");
	//printf("%s\n", "22222222222222222222222227");
	...
	//printf("<<<<<<<%d,%d,%s\n", dir_inode->i_num,newino->i_num,filename);
